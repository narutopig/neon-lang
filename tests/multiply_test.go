...
func TestMultiply(t *testing.T) {
	tests := []struct {
		...
		{"String * number", value.NewString("AMONGA US"), value.NewNumber(3), value.NewString("AMONGA USAMONGA USAMONGA US"), false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got, err := operations.Multiply(tt.left, tt.right); !reflect.DeepEqual(got, tt.want) && ((err != "") == tt.err) {
