...
func Lex(content string) ([]Token, ne.Error) {
	tokens := make([]Token, 0)
	...
	for i := 0; i < len(chars); i++ {
		...
		if currType == STRINGVALUE {
			...
			if char == "\"" {
				...
				if i+1 >= len(chars) {
					return tokens, ne.EndOfFileError(line, "reached end of file while parsing string")
				}
				return tokens, ne.EndOfLineError(line, "reached end of line while parsing string")
		...
		if sct.Type != NONE {
			...
			if sct.Type == PERIOD {
				if currType == NUMVALUE {
					if strings.Contains(currBlock, ".") {
						return tokens, ne.IllegalCharError(line, "illegal char .")
					}
					currBlock += "."
					continue
				}
			...
			if !isNum(char) {
				// not digit in num
				return tokens, ne.IllegalCharError(line, "unexpected character found while parsing number")
			}
			currBlock += char
		} else {
			// not in string
			if char == "\"" {
				...
				if isNum(char) {
					...
					return tokens, ne.IllegalCharError(line, fmt.Sprintf("unexpected character %s", char))
				}
				currBlock += char
